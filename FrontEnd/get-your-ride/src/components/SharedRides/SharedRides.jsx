...
function SharedRides({ onspinner }) {
    ...
    useEffect(() => {
        ...
        socket.on('receivemessage', (data) => {
            ...
            console.log(selectedRequestForChat);
    ...
    const handleSendMessage = () => {
        ...
        if (inputVal) {
            var data = {
                ...
                Sender: userInfo._id,
                Receiver: selectedRequestForChat.Seeker._id,
                Message: inputVal,
        ...
        <div>
            ...
            <div id='request-chat-modal' style={{ "boxShadow": "0 0 #0000, 0 0 #0000, 0 1px 2px 0 rgb(0 0 0 / 0.05)" }}
                ...
                <div className="relative flex flex-col space-y-1.5 pb-2 border-b-[1px]">
                    <h2 className="font-semibold text-lg tracking-tight">{selectedRequestForChat?.Seeker?.firstName} {selectedRequestForChat?.Seeker?.lastName}</h2>
                ...
                <div className="flex flex-col pr-4 md:h-[350px] h-2xl my-1 overflow-y-scroll" style={{ "minWidth": "100%" }}>
                    {
                            ...
                            <div key={index} className='mt-2 w-full'>
                                <div className={`inline-flex max-w-[320px] flex-col p-2 shadow-sm ${item.Receiver == userInfo._id ? 'rounded-e-xl rounded-es-xl bg-gray-100' : 'rounded-s-xl  rounded-ee-xl bg-blue-100 float-end' } dark:bg-gray-700`}>
                                    <div className={`flex items-center ${item.Sender == userInfo._id ? 'justify-end' : ''} space-x-2 rtl:space-x-reverse`}>
                                        <span className="text-xs font-semibold text-gray-900 dark:text-white">{item.Receiver == userInfo._id ? selectedRequestForChat?.Seeker?.firstName : 'You'}</span>
                                    </div>
